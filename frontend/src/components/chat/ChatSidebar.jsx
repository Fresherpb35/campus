...
const ChatSidebar = ({ chats, activeChat, onSelectChat, currentUser, activeTab, setActiveTab, isMobile=false, isOpen=true, onClose=()=>{} }) => {
  // Render overlay on mobile when `isMobile` and `isOpen` are true
  if (isMobile && !isOpen) return null;

  const containerClass = isMobile
    ? 'fixed inset-y-0 left-0 w-full max-w-sm z-50 transform transition-transform'
    : 'w-full md:w-80';
  ...
  return (
    <div>
      ...
      <div 
        className={`${containerClass} border-r flex flex-col h-full overflow-hidden transition-colors duration-300 bg-white`} 
        style={{ 
      ...
      >
        {isMobile && (
          <div className="p-4 border-b flex items-center justify-between" style={{ borderColor: 'var(--mui-palette-divider)' }}>
            <div className="font-black uppercase">Messages</div>
            <button onClick={onClose} className="p-2 rounded-md bg-transparent">
